...
function get_profiles {
	  ...
	  then
			 ...
			 #for item in `ls $PROFILEDIR | cut -d \. -f 1 | sort -u 2>>$LOG_FILE`
			 do
...
function get_BCFile {
	  ...
	  # @param1: directory with all files for profiles items
	  BCDIR=${1:-"NONE"}
	  MASTERBCFILE=`find $BCDIR -type f -regex '.*.bc'`
	  if [ ! -f $MASTERBCFILE ]; then
			 printf "CAN\'T FIND BOUNDARY CONDITION FILE!\n"
			 exit 1
	  fi
}
...
get_BCFile "${SCRPROPERTY[$PROFILESDIR_ID]}"
...
#			 fi
			 ...
			 GEOMTURBOFILE=`find ${SCRPROPERTY[$FINALDIR_ID]}/$PROFILEWDR/$INPUTSDIR -type f -iname "*$GEOMTURBOEXT"`
...
#						  head -n 1 $BCILE > $BOUNDARYCND_DIR/$REYNODIR/$profile$BCEXT
#						  awk '$2 == '$REYNO' {print $0}' $BCFILE >> $BOUNDARYCND_DIR/$REYNODIR/$profile$BCEXT 2>>$LOG_FILE
						  awk '{print $0}' $BCFILE >> $BOUNDARYCND_DIR/$REYNODIR/$profile$BCEXT 2>>$LOG_FILE
