...
## Generate file with changes for .def file - positive ND
	  ...
	  awk -f $AWKSCRIPT -v IN_MODEFILE=$NEWFILE -v IN_NDIAM=$NODALDIAMETER -v IN_FREQ=$FREQUENCY $DEFFILETEMPL > $DEF_NEW_NAME".dat"
	  ...
	  ## Generate file with changes for .def file - negative ND
			 ...
			 awk -f $AWKSCRIPT -v IN_MODEFILE=$NEWFILE -v IN_NDIAM=$(expr $NODALDIAMETER \* \-1) -v IN_FREQ=$FREQUENCY $DEFFILETEMPL > $NEGDEF_NEW_NAME".dat"
