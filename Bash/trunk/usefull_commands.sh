...
awk 'BEGIN{xmin=10000000; xmax=-1000000;};{if($1 ~ /850.00/){ if(xmin > $2){xmin=$2}; if(xmax < $2){xmax=$2} }}END{print "Xmin = ",xmin,"\nXmax = ",xmax}' ./M5_50_LSB-0b.geomTurbo

lmutil lmstat -c 27002@pwrs-lic02.corp.doosan.com -i > ./licence_status.nfo

for item in `find ./ -regex '.*100perc.*.[0-9]+kPa.*.3Ddomains_quantities.*'`; do BASENAME=$(basename $item); echo "CASE $BASENAME PROCESSING..."; echo "$BASENAME" >> "Cases_from_2.5kPa_to_7kPa.dat"; cat $item >> "Cases_from_2.5kPa_to_7kPa.dat"; done;

 FIRSTLINE=1; OUTFILE="Cases_from_2.5kPa_to_7kPa_nearest.dat"; for item in `find ./ -regex '.*100perc.*.[0-9]+kPa.*.3Ddomains_quantities.*'`; do BASENAME=$(basename $item);if [ $FIRSTLINE -eq 1 ]; then sed -n '16p' < $item >> $OUTFILE; FIRSTLINE=0; fi; sed -n '17p' < $item >> $OUTFILE; done;

FIRSTLINE=1; OUTFILE="Cases_from_2.5kPa_to_7kPa_half.dat"; for item in `find ./ -regex '.*100perc.*.[0-9]+kPa.*.3Ddomains_quantities.*'`; do BASENAME=$(basename $item);if [ $FIRSTLINE -eq 1 ]; then sed -n '16p' < $item >> $OUTFILE; FIRSTLINE=0; fi; sed -n '21p' < $item >> $OUTFILE; done;

find ./ -type f -regex '.*.cfdpost.cas' -exec /opt/sw/ansys_inc-15/v150/CFD-Post/bin/cfx5post -batch "/windows/D/Codes/Ansys_templ/CFDPostcse/2DProfile_results_exports_SST_xy_aveAbs.cse" -res '{}' \;

find ./ -type f -regex '.*.cfdpost.cas' -exec /opt/sw/ansys_inc-15/v150/CFD-Post/bin/cfx5post -batch "/windows/D/Codes/Ansys_templ/CFDPostcse/2DProfile_results_exports_SST_xy_aveAbs.cse" -res '{}' \;


mkdir angle80.7 angle81.7 angle82.7 angle83.7 angle84.2 angle84.7

grep -r -C 6 'get_poly' ./

./run_M4_LSN0_shaper.sh /opt/sw/MATLAB/MATLAB_Compiler_Runtime/v715 finalTest1

tar -cvzf M4_shaper.tar.gz *

#convert filename from utf-8 to cp1250 
convmv -r --notest -f cp1250 -t utf-8 ./


cfview -macro /home/mnadvornik/mswin/Codes/doosanskodapower/CFVIEW_tools/trunk/Stage_evaluation_steam.py -batch -project /home/mnadvornik/EXPERIMENTS/2015/M5_2stages_withlong_output/M5_ventilace_long_output/M5_ventilace_long_output_grid4_20perc_main/M5_ventilace_long_output_grid4_20perc_main.run -print


igg -autogrid5 -print >> $(date +%d%m%Y)_processlog.log

igg -autogrid5 -batch -trb /home/mnadvornik/EXPERIMENTS/2015/2015_001_SPW_Modul5_Axial_output_ventilace/SOLUTION/Mesh/last_2stages_extraout/Modul5_2stages_extra_outlet_template.trb -geomturbo /home/mnadvornik/EXPERIMENTS/2015/2015_001_SPW_Modul5_Axial_output_ventilace/SOLUTION/Mesh/last_2stages_extraout/Modul5_2stages_extra_outlet_ver8.geomTurbo -mesh /home/mnadvornik/EXPERIMENTS/2015/2015_001_SPW_Modul5_Axial_output_ventilace/SOLUTION/Mesh/last_2stages_extraout/Modul5_2stages_extra_outlet_ver8.igg >> $(date +%d%m%Y)_automeshwrkflow.log


awk 'BEGIN {IGNORECASE=1;}($0 ~ /part/) { print $0; }' ./2014_117_TG10_s_ucp_8stg_steam_computation_1_11bar.wall > ./2014_117_TG10_s_ucp_8stg_steam_computation_1_11bar.wallfilter
