...
declare DZETA_YRANGE="set yrange[0.01:0.12]\n"
declare INCIDENCE_XRANGE="set xrange[-20:20]\n"
declare XRANGE_AUTOMATIC="set autoscale x\n"
...
function plot_graphsDependReynolds { 
	  ...
	  if [ $COUNTSOURCE -ge 1 ]
	   then
		 ...
		 #graph Dzeta-Ma
			 echo "GENERATE GRAPHS DZETA - MACH FOR ALL REYNOLDS WITHOUT INCIDENCE INFLUENCE"

			 XLABEL="Ma $one"
			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_dzeta_mach"
			 TITLE_GRAPH="Loss coeficient from temperature for $BLADENAME"
			 YLABEL="Dzeta$one"

			 Create2DGraph "$TITLE_GRAPH" "$XLABEL" "$YLABEL" "$id_Mach" "$id_Dzeta" \
			 "$ACTUALPLOTSCRIPT" "$OUTPUT_LINE" "$REYNOTITLE"


  		 #graph Y-Ma
#			 echo "GENERATE GRAPHS Y - MACH FOR ALL REYNOLDS WITHOUT INCIDENCE INFLUENCE"	

			 #popis vlevo nahore
#			 printf "set key top left\n" >>$ACTUALPLOTSCRIPT 2>>$LOG_FILE
#			 printf "set autoscale y\n" >>$ACTUALPLOTSCRIPT 2>>$LOG_FILE

#			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_Y_mach"
#			 TITLE_GRAPH="Loss coeficient from pressure for $BLADENAME"
#			 YLABEL="Y$one"

#			 Create2DGraph "$TITLE_GRAPH" "$XLABEL" "$YLABEL" "$id_Mach" "$id_DzetaY" \
#			 "$ACTUALPLOTSCRIPT" "$OUTPUT_LINE" "$REYNOTITLE"

		 #graph Dzeta_kin-Ma
			 echo "GENERATE GRAPHS DZETA KIN - MACH FOR ALL REYNOLDS WITHOUT INCIDENCE INFLUENCE"

			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_dzetakin_mach"
			 TITLE_GRAPH="Loss coeficient from velocity for $BLADENAME"
			 YLABEL="Dzeta_kin$one"

			 Create2DGraph "$TITLE_GRAPH" "$XLABEL" "$YLABEL" "$id_Mach" "$id_kinloss" \
			 "$ACTUALPLOTSCRIPT" "$OUTPUT_LINE" "$REYNOTITLE"


  		 #graph OutAngle-Ma
			 echo "GENERATE GRAPHS BETAOUT - MACH FOR ALL REYNOLDS WITHOUT INCIDENCE INFLUENCE"	
			 ...
			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_outangle_mach"
			 TITLE_GRAPH="Output angle for $BLADENAME"
			 YLABEL="Beta_out$angle"

			 Create2DGraph "$TITLE_GRAPH" "$XLABEL" "$YLABEL" "$id_Mach" "$id_angle" \
			 "$ACTUALPLOTSCRIPT" "$OUTPUT_LINE" "$REYNOTITLE"

  		 #graph S-Ma
			 echo "GENERATE GRAPHS ENTROPHY - MACH FOR ALL REYNOLDS WITHOUT INCIDENCE INFLUENCE"	
			 ...
			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_entropy_mach"
			 TITLE_GRAPH="Entropy for $BLADENAME"
			 YLABEL="S$entrophy"

			 Create2DGraph "$TITLE_GRAPH" "$XLABEL" "$YLABEL" "$id_Mach" "$id_entrophy" \
			 "$ACTUALPLOTSCRIPT" "$OUTPUT_LINE" "$REYNOTITLE"

  		 #graph absT-Ma
			 echo "GENERATE GRAPHS ABSOLUTE TEMPERATURE - MACH FOR ALL REYNOLDS WITHOUT INCIDENCE INFLUENCE"	

			 #popis vlevo nahore
			 #printf "set key top left\n" >>$ACTUALPLOTSCRIPT 2>>$LOG_FILE
			 #printf "set autoscale y\n" >>$ACTUALPLOTSCRIPT 2>>$LOG_FILE

			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_absT_mach"
			 TITLE_GRAPH="Absolute temperature for $BLADENAME"
			 YLABEL="T_abs$kelvin"

			 Create2DGraph "$TITLE_GRAPH" "$XLABEL" "$YLABEL" "$id_Mach" "$id_absT" \
			 "$ACTUALPLOTSCRIPT" "$OUTPUT_LINE" "$REYNOTITLE"

  		 #graph |v|-Ma
			 echo "GENERATE GRAPHS VELOCITY MAGNITUDE - MACH FOR ALL REYNOLDS WITHOUT INCIDENCE INFLUENCE"	

			 #popis vlevo nahore
			 #printf "set key top left\n" >>$ACTUALPLOTSCRIPT 2>>$LOG_FILE
			 #printf "set autoscale y\n" >>$ACTUALPLOTSCRIPT 2>>$LOG_FILE

			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_magnitudeV_mach"
			 TITLE_GRAPH="Velocity magnitude for $BLADENAME"
			 YLABEL="|V|$speed"

			 Create2DGraph "$TITLE_GRAPH" "$XLABEL" "$YLABEL" "$id_Mach" "$id_absV" \
			 "$ACTUALPLOTSCRIPT" "$OUTPUT_LINE" "$REYNOTITLE"

  		 #graph Pstat-Ma
			 echo "GENERATE GRAPHS STATIC PRESSURE - MACH FOR ALL REYNOLDS WITHOUT INCIDENCE INFLUENCE"	

			 #popis vlevo nahore
			 printf "set key bottom left\n" >>$ACTUALPLOTSCRIPT 2>>$LOG_FILE
			 printf "set logscale y\n" >>$ACTUALPLOTSCRIPT 2>>$LOG_FILE

			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_Pstat_mach"
			 TITLE_GRAPH="Static pressure for $BLADENAME"
			 YLABEL="log P_stat$pascal"

			 Create2DGraph "$TITLE_GRAPH" "$XLABEL" "$YLABEL" "$id_Mach" "$id_statP" \
			 "$ACTUALPLOTSCRIPT" "$OUTPUT_LINE" "$REYNOTITLE"

  		 #graph Pabs-Ma
			 echo "GENERATE GRAPHS ABSOLUTE PRESSURE - MACH FOR ALL REYNOLDS WITHOUT INCIDENCE INFLUENCE"	

			 #popis vlevo nahore
			 printf "set key bottom left\n" >>$ACTUALPLOTSCRIPT 2>>$LOG_FILE			 

			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_Pabs_mach"
			 TITLE_GRAPH="Absolute pressure for $BLADENAME"
			 YLABEL="log P_abs$pascal"

			 Create2DGraph "$TITLE_GRAPH" "$XLABEL" "$YLABEL" "$id_Mach" "$id_absP" \
			 "$ACTUALPLOTSCRIPT" "$OUTPUT_LINE" "$REYNOTITLE"
	  ...
	  if [ $COUNTSOURCE -ge 1 ]
	   then
		 ...
		 #graph Dzeta-Ma
			 ...
			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_"$REYNOLDSTL"_dzeta_temp_inc"
			 TITLE_GRAPH="Loss coeficient from temperature for $BLADENAME"
			 YLABEL="Dzeta$one"
		 ...
		 #graph Dzeta_kin-Ma
			 echo "GENERATE GRAPHS KINLOSS - INCIDENCE FOR ALL INCIDENCE INFLUENCE"

			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_"$REYNOLDSTL"_dzeta_kin_inc"
			 TITLE_GRAPH="Loss coeficient from velocity for $BLADENAME"
			 YLABEL="Dzeta_kin$one"

			 Create2DGraph "$TITLE_GRAPH" "$XLABEL" "$YLABEL" "$id_incidence" "$id_kinloss" \
			 "$ACTUALPLOTSCRIPT" "$OUTPUT_LINE" "$REYMACHTITLE"
...
function plot_graphsSurface {
	  ...
	  if [ $COUNTSOURCE -ge 1 ]
	   then
			 #set logarithm scale for x
			 printf "set xtics 5\n" >>$ACTUALPLOTSCRIPT 2>>$LOG_FILE
			 REYNOLDS=$(expr "${ARRAYREDUCESOURCE[0]}" : '.*\(Re[0-9][0-9][0-9][0-9].*e3\)')
		 ...
		 #graph X-Cp
			 ...
			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_"$REYNOLDS"_bladeX_Cp"
			 TITLE_GRAPH="Press coeficient Cp on surface for $BLADENAME and $REYNOLDS"
			 YLABEL="Cp$one"
		 ...
		 #graph X-Maizo
			 ...
			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_"$REYNOLDS"_bladeX_MaIzo"
			 TITLE_GRAPH="Izoentropic Mach on surface for $BLADENAME and $REYNOLDS"
			 YLABEL="Ma_izo$one"
		 ...
		 #graph X-T stat
			 ...
			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_"$REYNOLDS"_bladeX_statTemp"
			 TITLE_GRAPH="Static Temperature on surface for $BLADENAME and $REYNOLDS"
		 ...
		 #graph X-Yplus
			 ...
			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_"$REYNOLDS"_bladeX_yplus"
			 TITLE_GRAPH="Y+ on surface for $BLADENAME and $REYNOLDS"
			 YLABEL="Y+$one"
		 ...
		 #graph X-Tau_w
			 ...
			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_"$REYNOLDS"_bladeX_tauW"
			 TITLE_GRAPH="Tau_w on surface for $BLADENAME and $REYNOLDS"
			 YLABEL="Tau_w$pascal"
		 ...
		 #graph X-U*
			 ...
			 OUTPUT_LINE=$GRAPHDIR/$BLADENAME"_"$REYNOLDS"_bladeX_fricU"
			 TITLE_GRAPH="U* on surface for $BLADENAME and $REYNOLDS"
			 YLABEL="U*$one"
...
function create_arrays_ReMa_inc {
	  #create array with all Reynolds numbers and Mach numbers     Ma0[0-9]*i0*
	  #prikaz najde vsechny adresare odpovidajici masce, vezme prvni sloupec nazvu (Re01000e3), seradi a odstrani duplicity
	  ...
	  ARRAYINDEX=0
	  for MACHS in `find $RESULTDIR -type f -name "*Ma0[0-9][0-9]*i[N-P][0-9][0-9]*aver*" -printf '%f\n' | cut -d \_ -f 2 | sort -u 2>>$LOG_FILE`
	  do	
			 #echo $MACHS		 
			 #MACHS=$(expr $MACHS : '.*\(Ma[0-9]\{2\}\)'|cut -d \_ -f 2)
			 #echo $MACHS
...
echo "ARRAYS OF REYNOLDS AND MACHS NUMBERS FOR INCIDENCE TYPE IS GENERATING....WAIT..."
#create array with all Reynolds numbers and Mach numbers
create_arrays_ReMa_inc
...
#BLADENAME=$(expr "$RESULTDIR" : '.*\(R[A-Z][a-z]\(_[0-9]\{3\}\)\{4\}\+\(_[a-z]\{3\}\)\?\)')
#BLADENAME=$(expr "$RESULTDIR" : '.*\(VS33.*.[0-9].[0-9]\{2\}\)')
#BLADENAME=$(expr "$RESULTDIR" : '.*\(Amager...[0-9].[0-9]\{2\}\)')
BLADENAME=$(expr "$RESULTDIR" : '.*\(T03a.*.[0-9].[0-9]\{2\}\)')
...
# negative incidence
			 ...
			 do
					...
					GNPL_INDEX_MAX=$(wc -l $item | cut -d ' ' -f 1)
					let GNPL_INDEX_MAX=$(($GNPL_INDEX_MAX-9))
...
do	  
	  REYNOLDS=${ARRAYOFREYNOLDS[$ARRAYINDEX]}
	  HEADER_INSERT=0
	  REDUCE_DATASOURCE=$GRAPHDIR$REYNOLDS$REDUCESOURCE
	  #create datasource from all runs without incidence influence	  
	  for item in `find $RESULTDIR -type f -name "*$REYNOLDS*$ZEROINCIDENCE*$AVGRESULTS" | sort 2>>$LOG_FILE`
	  do
			 #echo $item
			 #Get header of columns
			 if [ $HEADER_INSERT -eq 0 ]
			 ...
			 head -n $GNPL_INDEX_MAX $item | tail -n 1 >> $REDUCE_DATASOURCE
	  done

	  if [ -f $REDUCE_DATASOURCE ]
	  then
			 ARRAYREDUCESOURCE[ARRAYINDEX]=$REDUCE_DATASOURCE
	  fi
...
#Get all data from all runs - averaged values
ARRAYINDEX=0
while [ "$ARRAYINDEX" -lt "$SUMARRAYOFMACHS" ]
do	  
	  MACH=${ARRAYOFMACHS[$ARRAYINDEX]}
	  HEADER_INSERT=0
	  REDUCE_DATASOURCE=$GRAPHDIR$MACH$REDUCESOURCE
	  #create datasource from all runs without incidence influence	  
	  for item in `find $RESULTDIR -type f -name "*$MACH*$ZEROINCIDENCE*$AVGRESULTS" | sort 2>>$LOG_FILE`
	  do
			 #echo $item
			 #Get header of columns
			 if [ $HEADER_INSERT -eq 0 ]
...
do	  
	  ...
	  ARRAYINDEX=0
	  #create datasource from all runs without incidence influence	  
	  for item in `find $RESULTDIR -type f -name "*$REYNOLDS*$ZEROINCIDENCE*$SURFRESULTS" | sort 2>>$LOG_FILE`
	  do
			 #echo $item
